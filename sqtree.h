...
class SQtree {
 ...
 public:
   ...
   * pick the best partition of a Node of size w x h in O(wh) ti
   me.
 ...
 private:
  ...
  /* BEGIN ============== Your personal private functions ============= */
    ...
    Node* copyTraversal(Node *toCopy);
    void renderNode(PNG* img, Node *node);
    int sizeTraversal(Node* node);
    void clearTraversal(Node* node);
    double findLargestVariance(stats &s, pair<int, int> ul, int w, int h, int x, int y);
